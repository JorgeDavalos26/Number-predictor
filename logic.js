...
// ----------------------------------------------------
// Customize the canvas area upon first call of JS file
...
(function() {
    ...
    drawingcanvas = this.__canvas = new fabric.Canvas('bigcanvas', { isDrawingMode: true });
...
(async () => {
    modelSeq = await tf.loadLayersModel("modelSeq.json");
    console.log("Model has been loaded - Sequential");
})();


// ----------------------------------------------------
// Clean and Predict functionalities
...
function predict(conv) {
    ...
    if (conv) {
        var results = modelConv.predict(tensor4).dataSync();
    }
    else {
        var results = modelSeq.predict(tensor4).dataSync();
    }
    
    var prediction = results.indexOf(Math.max.apply(null, results));
    
    if (conv) {
        console.log("Prediccion convolucional", prediction);
        document.getElementById("divResultConv").innerHTML = prediction;
    }
    else {
        console.log("Prediccion secuencial", prediction);
        document.getElementById("divResultSeq").innerHTML = prediction;
    }
